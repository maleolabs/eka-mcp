...
Loss model: deletion is immediate and permanent (no trash). Bulk filters (`--project/--level`) cross every project in the workspace — verify with `eka shr list --level L0 --project <name>` first, then delete single explicit forms.
